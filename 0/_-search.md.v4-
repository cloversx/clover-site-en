...
(() => {
  ...
  function parseQuery(raw) {
    ...
    // IMPORTANT: tokenize (words + phrases) instead of split-only
    const textTokens = tokenizeText(textParts.join(" "));
  ...
  function scoreItem_AND(item, textTokens) {
    ...
    for (const tok of textTokens) {
      ...
      // Phrase/word both use includes (simple + fast).
