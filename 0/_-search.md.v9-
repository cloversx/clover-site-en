...
(() => {
  ...
  function renderTagPanel() {
    ...
    for (const it of finalList) {
      ...
      btn.addEventListener("click", () => {
        ...
        run();          // <-- this is crucial (keeps filters strict)
        $q.focus();
      });
  ...
  function render(items, rawQuery, parsed) {
    ...
    for (const it of items) {
      ...
      // ✅ restore tags under each result (clickable)
  ...
  function run() {
    ...
    // ✅ keep tag panel in sync with toggles (counts change with archives/current)
    renderTagPanel();
